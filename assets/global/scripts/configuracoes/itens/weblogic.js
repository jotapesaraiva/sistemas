...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/weblogic_list",//json datasource
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar weblogic'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/weblogic_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_weblogic);
            $('[name="ambiente"]').selectpicker('val', data.id_ambiente);
            $('[name="contexto"]').selectpicker('val', data.id_contexto);
            $('[name="tecnologia"]').selectpicker('val', data.id_tecnologia);
            $('[name="servidor"]').selectpicker('val', data.id_servidor);
            $('[name="weblogic"]').val(data.nome_weblogic);


            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar weblogic'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/weblogic_add";
    } else {
        url = href+"/weblogic_update";
    }
