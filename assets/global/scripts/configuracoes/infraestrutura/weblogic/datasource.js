...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/datasource_list",//json datasource
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar datasource'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/datasource_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.conexao.id_datasource);
            $('[name="nome"]').val(data.conexao.nome_datasource);
            $('[name="tipo"]').val(data.conexao.tipo_datasource);
            $('[name="jndi"]').val(data.conexao.jndi_datasource);
            $('[name="usuario"]').val(data.conexao.usuario_datasource);
            $('[name="string"]').val(data.conexao.string_url_datasource);

            $('[name="cluster[]"]').selectpicker('val', data.alvo);
            $('[name="weblogic"]').selectpicker('val', data.id_weblogic);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar datasource'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/datasource_add";
    } else {
        url = href+"/datasource_update";
    }
