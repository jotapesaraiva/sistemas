...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/managed_server_list",//json managed_server
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar Configuração de managed_server'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/managed_server_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_managed_server);
            $('[name="weblogic"]').selectpicker('val', data.id_weblogic);
            $('[name="certificado"]').selectpicker('val', data.id_certificado);
            $('[name="managed_server"]').val(data.nome_managed_server);
            $('[name="cluster"]').selectpicker('val', data.id_cluster);
            $('[name="porta"]').val(data.porta_managed_server);
            $('[name="versao_java"]').val(data.versao_java_managed_server);
            $('[name="memoria"]').val(data.memoria_managed_server);
            $('[name="argumento"]').val(data.argumento_managed_server);
            if(data.status_managed_server== '1') {
            ...
            $('.modal-title').text('Editar managed_server'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/managed_server_add";
    } else {
        url = href+"/managed_server_update";
    }
...
function delete_person(id){
    if(confirm('Você tem certeza que quer deletar o item?')) {
        ...
        $.ajax({
            url : href+"/managed_server_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
