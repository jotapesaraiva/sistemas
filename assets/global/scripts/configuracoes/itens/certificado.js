...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/certificado_list",//json datasource
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar certificado'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/certificado_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_certificado);
            $('[name="tipo"]').selectpicker('val', data.id_tipo_certificado);
            $('[name="documentacao"]').val(data.documentacao_certificado);
            $('[name="nome"]').val(data.nome_certificado);
            $('[name="url"]').val(data.url_certificado);
            $('[name="validade"]').val(data.validade_certificado);
            $('[name="diretorio"]').val(data.diretorio_certificado);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar certificado'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/certificado_add";
    } else {
        url = href+"/certificado_update";
    }
...
function delete_person(id){
    if(confirm('Você tem certeza que quer deletar o item?')) {
        ...
        $.ajax({
            url : href+"/certificado_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
