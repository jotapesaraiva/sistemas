...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/applications_list/"+ value,//json datasource
    ...
    $('select#selector').change(function(){
        value = $(this).val();
        console.log(value);
        table.ajax.url( href+"/applications_list/"+ value ).load();
    });
});
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar aplicação'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/applications_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.applications.id_applications);
            $('[name="sistema"]').selectpicker('val', data.applications.id_sistema);
            $('[name="aplicacao"]').val(data.applications.nome_applications);
            $('[name="versao"]').val(data.applications.versao_applications);
            $('[name="cluster"]').selectpicker('val', data.alvo);
            $('[name="weblogic"]').selectpicker('val', data.applications.id_weblogic);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar aplicação'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/applications_add";
    } else {
        url = href+"/applications_update";
    }
...
function delete_person(id){
    if(confirm('Você tem certeza que quer deletar a aplicação ?')) {
        // ajax delete data to database
        $.ajax({
            url : href+"/applications_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
