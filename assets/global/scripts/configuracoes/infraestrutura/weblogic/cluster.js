...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/cluster_list",//json cluster
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar Configuração de Cluster'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/cluster_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_cluster);
            $('[name="servidor"]').selectpicker('val', data.id_servidor);
            $('[name="certificado"]').selectpicker('val', data.id_certificado);
            $('[name="cluster"]').val(data.nome_cluster);
            $('[name="managed_server"]').val(data.managed_server_cluster);
            $('[name="porta"]').val(data.porta_cluster);
            $('[name="versao_java"]').val(data.versao_java_cluster);
            $('[name="memoria"]').val(data.memoria_cluster);
            $('[name="argumento"]').val(data.argumento_cluster);
            $('[name="status"]').val(data.status_cluster);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar cluster'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/cluster_add";
    } else {
        url = href+"/cluster_update";
    }
