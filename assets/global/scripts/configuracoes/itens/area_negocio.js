...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/area_negocio_list",//json datasource
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar area_negocios'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/area_negocio_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_negocio);
            $('[name="nome"]').val(data.nome_negocio);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar area_negocios'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/area_negocio_add";
    } else {
        url = href+"/area_negocio_update";
    }
...
function delete_person(id){
    if(confirm('Você tem certeza que quer deletar o item?')) {
        ...
        $.ajax({
            url : href+"/area_negocio_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
