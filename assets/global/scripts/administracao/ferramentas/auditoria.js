...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/auditoria_list",//json datasource
...
function add_person() {
    ...
    $('#modal_auditoria').modal('show'); // show bootstrap modal
    $('.modal-title').text('Adicionar auditoria'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/auditoria_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            console.log(data);
            $('[name="id"]').val(data.id_auditoria);
            $('[name="nome"]').val(data.nome_auditoria);
            $('[name="email"]').val(data.email_auditoria);
            $('[name="login"]').val(data.login_auditoria);
            $('[name="ativo"]').val(data.ativo_auditoria);
            $('[name="perfil"]').selectpicker('val', data.id_perfil);

            $('#modal_auditoria').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar auditoria'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/auditoria_add";
    } else {
        url = href+"/auditoria_update";
    }
