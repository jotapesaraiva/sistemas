...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "ajax": {
            url : href+"/repositorio_list",//json datasource
...
function add_person() {
    ...
    $('.modal-title').text('Adicionar repositorio'); // Set Title to Bootstrap modal title
}
...
function edit_person(id) {
    ...
    $.ajax({
        url : href+"/repositorio_edit/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            $('[name="id"]').val(data.id_repositorio);
            $('[name="nome"]').val(data.nome_repositorio);

            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Editar repositorio'); // Set title to Bootstrap modal title
        },
...
function save(){
    ...
    var url;
    if(save_method == 'add') {
        url = href+"/repositorio_add";
    } else {
        url = href+"/repositorio_update";
    }
...
function delete_person(id){
    if(confirm('Você tem certeza que quer deletar o item?')) {
        ...
        $.ajax({
            url : href+"/repositorio_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
